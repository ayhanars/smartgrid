 ...
 * 5°, or 1° with Shift.
 */
export function AngleWheel({ value, onChange, onStart, onEnd, size = 96, label = 'Angle' }: { value: number; onChange: (deg: number) => void; onStart?: () => void; onEnd?: () => void; size?: number; label?: string }) {
  ...
  const angleFrom = (e: React.PointerEvent) => {
    ...
    return Math.max(-90, Math.min(90, Math.round(deg / step) * step))
  }
  ...
  return (
    ...
    >
      ...
      <line className="angle-wheel__needle" x1={C - dx} y1={C - dy} x2={C + dx} y2={C + dy} />
